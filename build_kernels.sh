...
for kernel in $kernels; do
	...
	KCONFIG_NOTIMESTAMP=1 KBUILD_BUILD_TIMESTAMP='' KBUILD_BUILD_USER=chronos KBUILD_BUILD_HOST=localhost make LLVM=1 -C "./kernels/$kernel" -j"$NTHREADS" O=out || { echo "Kernel build failed"; exit 1; }
	...
	make LLVM=1 -C "./kernels/$kernel" -j"$NTHREADS" O=out INSTALL_MOD_STRIP=1 INSTALL_MOD_PATH="./kmods" modules_install
